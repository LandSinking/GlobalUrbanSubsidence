...
def main():
    ...
    median_values = data['Vm (mm/yr)']
    pth5_values = data['V5th (mm/yr)']
    p10_values = data['P10 (%)']
    ...
    sorted_data0 = data.sort_values(by='Vm (mm/yr)', ascending=False)


    sorted_latitudes0 = sorted_data0['Lat.(°N)']
    sorted_longitudes0 = sorted_data0['Lon.(°E)']
    sorted_median_values = sorted_data0['Vm (mm/yr)']
    ...
    sorted_data1 = data.sort_values(by='V5th (mm/yr)', ascending=False)

    sorted_latitudes1 = sorted_data1['Lat.(°N)']
    sorted_longitudes1 = sorted_data1['Lon.(°E)']
    sorted_pth5_values = sorted_data1['V5th (mm/yr)']
    ...
    median_value1 = round(median_value1)  
    median_str = f"Median: {median_value1}".replace("-", "−")  

    
    inset_ax.text(median_value1 - 98, np.max(y_vals_pth5) * 0.8,
                median_str, fontsize=6, fontname='Arial', color='black', ha='left', va='top')
    ...
    # icks2 = [0, 0.1, 0.3, 0.5, 0.7, 0.9]  
    ticks2 = [0, 10, 30, 50, 70, 90]  
    ...
    median_value2 = round(median_value2)  
    median_str = f"Median: {median_value2}"  


    inset_ax.text(median_value2 + 6, np.max(y_vals_p10) * 0.8,  
              median_str, fontsize=6, fontname='Arial', color='black', ha='left', va='top')
