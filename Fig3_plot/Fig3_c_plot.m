...
for r = 1:10
    ...
    if mod(r-1, num_cols) == 0
        ylabel('Proportion (%)', 'FontName','Arial','FontSize',22,'FontWeight','bold');
    else
        set(gca, 'YTickLabel', []);
    end
    ...
    h = title(region_name, 'FontWeight','bold','FontName','Arial','FontSize',22);
    pos = get(h,'Position');
    ...
    if (row_idx <= 3 && col_idx == num_cols) || (row_idx == 4 && col_idx == 1)
        ylabel('Magnitude (mm/yr)','FontName','Arial','FontSize',22,'FontWeight','bold');
    else
        set(gca,'YTickLabel',[]);
    end
